...
public class WebActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(isPC) {
            mWebView.getSettings().setUserAgentString("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/48.0.2564.97 Safari/537.36");
        }
        if (mCanSetTitle) {
...
//                mWebView.loadJs("if(!!onresult)onresult('" + ret + "')");
    ...
    public void finish(String ret) {
        ...
        intent.putExtra( EXTRAS_KEY_RET, ret);
